...
**A causa foi localizada no programa do CLP.** Existe um controlador PID de vazao, `FIC796702`,
alimentado pelo transmissor `FIT796702`:

```
MOVE(FIT796702.Out, FIC796702.AUX_PV)      <- a vazao medida alimenta o PID
MOVE(FIC796702.IHM_SP, FIC796702.PID.SP)   <- o setpoint vem da IHM
```
...
> Quem definiu o setpoint de vazao na IHM, e com base em que? Se veio de calculo de velocidade no
> spray-ball, o numero esta certo e e a faixa da nota que precisa ser corrigida. Se ninguem sabe a
> origem, essa e a hora de definir.

O caminho mais rapido para confirmar o valor exato e olhar a tela de CIP no supervisorio. Pela via
do CLP a tag e `FIC796702.PID.SP`.
...
|---|---|---|---|
| 1 | Quem definiu o setpoint de vazao (~350 hl/h) na IHM e com base em que? | Processo / Automacao | 1,49 + 0,75 pts |
